...
Options:
  ...
  --auth-token-file PATH   Export MARMOT_AGENT_AUTH_TOKEN_FILE and pass it to wn-agent
  --socket-dir-mode OCTAL  wn-agent control socket directory mode (default: 0700)
  --socket-mode OCTAL      wn-agent control socket mode (default: 0600)
  --relay URL              Add a wn-agent --relay argument; may be repeated
...
After setup:
  ...
  ROOT/run-wn-agent.sh [extra wn-agent flags]
...
write_helper_scripts() {
    cat >"$dev_root/run-wn-agent.sh" <<'SCRIPT'
#!/usr/bin/env bash
set -euo pipefail
...
exec "$MDK_REPO/scripts/hermes_marmot_deterministic_e2e.sh" --root "$dev_root"
SCRIPT
...
set -euo pipefail
dev_root="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
for name in hermes-gateway wn-agent; do
