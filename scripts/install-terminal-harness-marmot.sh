...
# MARMOT_TERMINAL_HARNESS must be `pi`, `opencode`, or `prime-agent`.
...
case "$HARNESS_KIND" in
    ...
    opencode)
        ...
        ;;
    prime-agent)
        HARNESS_DISPLAY_NAME="Prime Agent"
        HARNESS_ENV_PREFIX="WN_PRIME_AGENT"
        HARNESS_DEFAULT_BIN="prime-agent"
        HARNESS_FALLBACK_BIN_DIR="$HOME/.local/bin"
        HARNESS_DEFAULT_HOME="$HOME/.marmot-agents/prime-agent"
        HARNESS_DEFAULT_AGENT_LABEL="prime-agent-harness-agent"
        HARNESS_DEFAULT_AGENT_SERVICE="wn-agent-prime-agent"
        HARNESS_DEFAULT_AGENT_LAUNCHD="org.marmot.wn-agent.prime-agent"
        ;;
    *)
        echo "error: MARMOT_TERMINAL_HARNESS must be pi, opencode, or prime-agent" >&2
        exit 64
        ;;
esac
...
read_persisted_env_value() {
    ...
    [ -f "$env_file" ] || return 1
    # This private file is generated by this installer as shell-escaped
    # assignments and is documented as sourceable. Read it in an isolated
    # shell so paths containing spaces round-trip without eval in this process.
...
HARNESS_REQUIRED_VERSION=""
HARNESS_EXTRA_ENV_NAME=""
HARNESS_EXTRA_ENV_VALUE=""
HARNESS_EXTRA_ENV_EXPLICIT=0
if [ "$HARNESS_KIND" = prime-agent ]; then
    HARNESS_REQUIRED_VERSION="$(env_or_default WN_PRIME_AGENT_REQUIRED_VERSION 0.7.0)"
    HARNESS_EXTRA_ENV_NAME="WN_PRIME_AGENT_DAEMON_SOCKET"
    HARNESS_EXTRA_ENV_VALUE="$(env_or_default "$HARNESS_EXTRA_ENV_NAME" "$MARMOT_HOME/dev/prime-agent.sock")"
    if [ -n "${WN_PRIME_AGENT_DAEMON_SOCKET:-}" ]; then
        HARNESS_EXTRA_ENV_EXPLICIT=1
    fi
fi
...
Environment:
  ...
  WN_PRIME_AGENT_REQUIRED_VERSION Required Prime Agent version (default: 0.7.0)
  WN_PRIME_AGENT_DAEMON_SOCKET Prime Agent daemon socket persisted into services
...
verify_harness_version() {
    [ -n "$HARNESS_REQUIRED_VERSION" ] || return 0
    if [ "$DRY_RUN" -eq 1 ]; then
        log "would verify $HARNESS_DISPLAY_NAME version: $HARNESS_REQUIRED_VERSION"
        return 0
    fi
    local actual actual_version
    if ! actual="$("$HARNESS_BIN" --version 2>&1)"; then
        echo "error: failed to read $HARNESS_DISPLAY_NAME version from $HARNESS_BIN" >&2
        exit 1
    fi
    actual_version="$(printf '%s\n' "$actual" | head -n 1 | tr -d '\r')"
    case "$actual_version" in
        "prime-agent v"*) actual_version="${actual_version#prime-agent v}" ;;
        "prime-agent "*) actual_version="${actual_version#prime-agent }" ;;
        v*) actual_version="${actual_version#v}" ;;
    esac
    if [ "$actual_version" != "$HARNESS_REQUIRED_VERSION" ]; then
        echo "error: $HARNESS_DISPLAY_NAME $HARNESS_REQUIRED_VERSION is required; got: $actual" >&2
        exit 1
    fi
}
...
install_macos_harness_service() {
    ...
    (
        ...
        if [ -n "$HARNESS_EXTRA_ENV_NAME" ]; then
            plist_env_entry "$HARNESS_EXTRA_ENV_NAME" "$HARNESS_EXTRA_ENV_VALUE"
        fi
        printf '%s\n' '  </dict>'
        printf '%s\n' '  <key>RunAtLoad</key>'
        printf '%s\n' '  <true/>'
...
install_linux_harness_service() {
    ...
    (
        ...
        if [ -n "$HARNESS_EXTRA_ENV_NAME" ]; then
            printf 'Environment=%s\n' "$(systemd_quote "$HARNESS_EXTRA_ENV_NAME=$HARNESS_EXTRA_ENV_VALUE")"
        fi
        printf 'ExecStart=%s\n' "$(systemd_quote "$program")"
        printf '%s\n' 'Restart=always'
        printf '%s\n' 'RestartSec=2'
...
write_harness_env() {
    ...
    (
        umask 077
        {
            ...
            if [ -n "$HARNESS_EXTRA_ENV_NAME" ]; then
                printf '%s=%q\n' "$HARNESS_EXTRA_ENV_NAME" "$HARNESS_EXTRA_ENV_VALUE"
            fi
        } >"$env_file"
    )
...
if [ "$HARNESS_KIND" = prime-agent ] && [ "$HARNESS_EXTRA_ENV_EXPLICIT" -eq 0 ]; then
    existing_harness_env="$MARMOT_HOME/dev/$HARNESS_BINARY.env"
    persisted_daemon_socket=""
    if persisted_daemon_socket="$(
        read_persisted_env_value "$existing_harness_env" "$HARNESS_EXTRA_ENV_NAME"
    )" && [ -n "$persisted_daemon_socket" ]; then
        HARNESS_EXTRA_ENV_VALUE="$persisted_daemon_socket"
    else
        HARNESS_EXTRA_ENV_VALUE="$MARMOT_HOME/dev/prime-agent.sock"
    fi
fi
