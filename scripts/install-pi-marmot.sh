...
# Install wn-agent and the wn-pi harness from a WN Agent GitHub release.
# Pi itself must already be installed.
...
MARMOT_HOME="${MARMOT_HOME:-${HOME}/.marmot-agents/pi}"
...
MARMOT_AGENT_LABEL="${MARMOT_AGENT_LABEL:-pi-harness-agent}"
MARMOT_AGENT_SERVICE_NAME="${MARMOT_AGENT_SERVICE_NAME:-wn-agent-pi}"
MARMOT_AGENT_LAUNCHD_LABEL="${MARMOT_AGENT_LAUNCHD_LABEL:-org.marmot.wn-agent.pi}"
MARMOT_RELAYS="${MARMOT_RELAYS:-wss://relay.eu.whitenoise.chat,wss://relay.us.whitenoise.chat}"
WN_PI_BIN="${WN_PI_BIN:-pi}"
WN_PI_TIMEOUT_SECS="${WN_PI_TIMEOUT_SECS:-3600}"
WN_PI_IDLE_TIMEOUT_SECS="${WN_PI_IDLE_TIMEOUT_SECS:-120}"
WN_PI_REQUEST_TIMEOUT_SECS="${WN_PI_REQUEST_TIMEOUT_SECS:-30}"
WN_PI_MAX_REPLY_BYTES="${WN_PI_MAX_REPLY_BYTES:-30000}"
WN_PI_MAX_PENDING_PER_GROUP="${WN_PI_MAX_PENDING_PER_GROUP:-4}"
...
usage() {
    cat <<'USAGE'
Usage: install-pi-marmot.sh [options]

Install wn-agent and wn-pi from a WN Agent GitHub release. Pi
...
Options:
  ...
  --home PATH              Marmot terminal harness agent home (default: ~/.marmot-agents/pi)
  ...
  --pi-bin PATH      Pi binary or command name (default: pi)
  ...
  --no-start-wn-pi   Install but do not start wn-pi
...
Environment:
  ...
  MARMOT_HOME              wn-agent home (default: ~/.marmot-agents/pi)
  MARMOT_AGENT_SOCKET      wn-agent socket (default: $MARMOT_HOME/dev/wn-agent.sock)
  MARMOT_AGENT_LABEL       Account label used by bootstrap (default: pi-harness-agent)
  MARMOT_AGENT_SERVICE_NAME Linux systemd user service name (default: wn-agent-pi)
  MARMOT_AGENT_LAUNCHD_LABEL macOS LaunchAgent label (default: org.marmot.wn-agent.pi)
  ...
  WN_PI_ALLOWED_SENDERS_HEX Comma-separated hex values for prompt senders
  WN_PI_BIN          Pi binary or command name

Example:
  curl -fsSL https://github.com/marmot-protocol/mdk/releases/download/wn-agent-latest/install-pi-marmot.sh | bash

  curl -fsSL .../install-pi-marmot.sh | bash -s -- --yes --allow-welcomer npub1...
USAGE
}

log() {
    printf 'install-pi-marmot: %s\n' "$*"
}

warn() {
    printf 'install-pi-marmot: warning: %s\n' "$*" >&2
}
...
resolve_pi_bin() {
    if [ "$DRY_RUN" -eq 1 ]; then
        log "would require Pi binary: $WN_PI_BIN"
        return 0
    fi
    if [[ "$WN_PI_BIN" == */* ]]; then
        if [ ! -x "$WN_PI_BIN" ]; then
            echo "error: WN_PI_BIN is not executable: $WN_PI_BIN" >&2
            exit 1
        fi
        return 0
    fi
    if command -v "$WN_PI_BIN" >/dev/null 2>&1; then
        WN_PI_BIN="$(command -v "$WN_PI_BIN")"
        return 0
    fi
    local pi_home_bin="$HOME/.pi/bin/$WN_PI_BIN"
    if [ ! -x "$pi_home_bin" ]; then
        echo "error: Pi binary not found: $WN_PI_BIN" >&2
        echo "install Pi first or set WN_PI_BIN/--pi-bin" >&2
        exit 1
    fi
    WN_PI_BIN="$pi_home_bin"
    log "found Pi binary: $WN_PI_BIN"
}
...
install_macos_pi_service() {
    local plist_dir plist label program logs_dir
    label="org.marmot.wn-pi"
    plist_dir="$HOME/Library/LaunchAgents"
    plist="$plist_dir/$label.plist"
    program="$(wn_pi_path)"
    logs_dir="$MARMOT_HOME/logs"
    ...
    if [ "$DRY_RUN" -eq 1 ]; then
        log "would install LaunchAgent $plist"
        log "would run: launchctl bootstrap gui/$UID $plist"
        log "env: WN_PI_TIMEOUT_SECS=$WN_PI_TIMEOUT_SECS WN_PI_IDLE_TIMEOUT_SECS=$WN_PI_IDLE_TIMEOUT_SECS"
        return 0
    fi
    ...
    run mkdir -p "$plist_dir" "$logs_dir" || return 1
    {
        ...
        plist_env_entry "WN_PI_ACCOUNT_ID_HEX" "$BOOTSTRAP_ACCOUNT_ID_HEX"
        plist_env_entry "WN_PI_ALLOWED_SENDERS_HEX" "$BOOTSTRAP_ALLOWED_SENDERS_HEX"
        plist_env_entry "WN_PI_BIN" "$WN_PI_BIN"
        plist_env_entry "WN_PI_TIMEOUT_SECS" "$WN_PI_TIMEOUT_SECS"
        plist_env_entry "WN_PI_IDLE_TIMEOUT_SECS" "$WN_PI_IDLE_TIMEOUT_SECS"
        plist_env_entry "WN_PI_REQUEST_TIMEOUT_SECS" "$WN_PI_REQUEST_TIMEOUT_SECS"
        plist_env_entry "WN_PI_MAX_REPLY_BYTES" "$WN_PI_MAX_REPLY_BYTES"
        plist_env_entry "WN_PI_MAX_PENDING_PER_GROUP" "$WN_PI_MAX_PENDING_PER_GROUP"
        printf '%s\n' '  </dict>'
        printf '%s\n' '  <key>RunAtLoad</key>'
        printf '%s\n' '  <true/>'
        printf '%s\n' '  <key>KeepAlive</key>'
        printf '%s\n' '  <true/>'
        printf '%s\n' '  <key>StandardOutPath</key>'
        plist_string "$logs_dir/wn-pi.out.log"
        printf '%s\n' '  <key>StandardErrorPath</key>'
        plist_string "$logs_dir/wn-pi.err.log"
        printf '%s\n' '</dict>'
        printf '%s\n' '</plist>'
    } >"$plist" || return 1
    chmod 600 "$plist" || return 1
...
install_linux_pi_service() {
    local service_dir service program
    service_dir="$HOME/.config/systemd/user"
    service="$service_dir/wn-pi.service"
    program="$(wn_pi_path)"

    if [ "$DRY_RUN" -eq 1 ]; then
        log "would install systemd user unit $service"
        log "would restart wn-pi.service when active; otherwise enable --now"
        log "env: WN_PI_TIMEOUT_SECS=$WN_PI_TIMEOUT_SECS WN_PI_IDLE_TIMEOUT_SECS=$WN_PI_IDLE_TIMEOUT_SECS"
        return 0
    fi
    ...
    {
        printf '%s\n' '[Unit]'
        printf '%s\n' 'Description=Marmot wn-pi harness'
        ...
        printf 'Environment=%s\n' "$(systemd_quote "WN_PI_ACCOUNT_ID_HEX=$BOOTSTRAP_ACCOUNT_ID_HEX")"
        printf 'Environment=%s\n' "$(systemd_quote "WN_PI_ALLOWED_SENDERS_HEX=$BOOTSTRAP_ALLOWED_SENDERS_HEX")"
        printf 'Environment=%s\n' "$(systemd_quote "WN_PI_BIN=$WN_PI_BIN")"
        printf 'Environment=%s\n' "$(systemd_quote "WN_PI_TIMEOUT_SECS=$WN_PI_TIMEOUT_SECS")"
        printf 'Environment=%s\n' "$(systemd_quote "WN_PI_IDLE_TIMEOUT_SECS=$WN_PI_IDLE_TIMEOUT_SECS")"
        printf 'Environment=%s\n' "$(systemd_quote "WN_PI_REQUEST_TIMEOUT_SECS=$WN_PI_REQUEST_TIMEOUT_SECS")"
        printf 'Environment=%s\n' "$(systemd_quote "WN_PI_MAX_REPLY_BYTES=$WN_PI_MAX_REPLY_BYTES")"
        printf 'Environment=%s\n' "$(systemd_quote "WN_PI_MAX_PENDING_PER_GROUP=$WN_PI_MAX_PENDING_PER_GROUP")"
        printf 'ExecStart=%q\n' "$program"
        printf '%s\n' 'Restart=always'
        printf '%s\n' 'RestartSec=2'
        printf '\n'
    ...
    run systemctl --user daemon-reload || return 1
    activate_systemd_user_service wn-pi.service || return 1
    log "installed and started systemd user service: wn-pi.service"
}
...
install_pi_service() {
    if [ "$NO_START_WN_PI" -eq 1 ]; then
        log "not starting wn-pi because --no-start-wn-pi was passed"
        return 0
    fi
    case "$(uname -s)" in
        Darwin) install_macos_pi_service ;;
        Linux) install_linux_pi_service ;;
        *) return 1 ;;
    esac
}
...
write_pi_env() {
    local env_file="$MARMOT_HOME/dev/wn-pi.env"
    if [ "$DRY_RUN" -eq 1 ]; then
    ...
    (
        umask 077
        {
            ...
            printf 'WN_PI_ACCOUNT_ID_HEX=%q\n' "$BOOTSTRAP_ACCOUNT_ID_HEX"
            printf 'WN_PI_ALLOWED_SENDERS_HEX=%q\n' "$BOOTSTRAP_ALLOWED_SENDERS_HEX"
            printf 'WN_PI_BIN=%q\n' "$WN_PI_BIN"
            printf 'WN_PI_TIMEOUT_SECS=%q\n' "$WN_PI_TIMEOUT_SECS"
            printf 'WN_PI_IDLE_TIMEOUT_SECS=%q\n' "$WN_PI_IDLE_TIMEOUT_SECS"
            printf 'WN_PI_REQUEST_TIMEOUT_SECS=%q\n' "$WN_PI_REQUEST_TIMEOUT_SECS"
            printf 'WN_PI_MAX_REPLY_BYTES=%q\n' "$WN_PI_MAX_REPLY_BYTES"
            printf 'WN_PI_MAX_PENDING_PER_GROUP=%q\n' "$WN_PI_MAX_PENDING_PER_GROUP"
        } >"$env_file"
    )
    log "wrote wn-pi env -> $env_file"
}
...
while [ "$#" -gt 0 ]; do
    case "$1" in
        ...
        --relay)
            ...
            ;;
        --pi-bin)
            WN_PI_BIN="${2:?missing value for --pi-bin}"
            shift 2
            ;;
        --no-service)
            INSTALL_SERVICE=0
...
if [ -n "${WN_PI_ALLOWED_SENDERS_HEX:-}" ]; then
    while IFS= read -r welcomer; do
        ALLOW_WELCOMERS+=("$welcomer")
    done < <(append_csv "$WN_PI_ALLOWED_SENDERS_HEX")
fi
...
install_binary_bundle "wn-pi" "$platform" "$tmpdir"
bootstrap_agent
write_pi_env

if [ "$INSTALL_SERVICE" -eq 1 ]; then
    install_pi_service || warn "wn-pi service was not installed; run it manually with $MARMOT_HOME/dev/wn-pi.env"
fi
