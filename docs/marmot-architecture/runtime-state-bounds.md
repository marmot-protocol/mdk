...
| Protected partial ciphertext (C8-C3) | One prefix per job, at most the configured transfer ceiling; writes at most 1 MiB; partial plus retained payload shares the account budget | Source/attempt/expiry fences on every write; terminal/parked state, success, source removal and explicit removal cascade cleanup. A 24-hour last-progress expiry is reclaimed in batches of 64 by active maintenance. Strong ETag, exact locator and ciphertext identity gate Range reuse. |
