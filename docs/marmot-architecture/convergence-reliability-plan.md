...
| A11 | exact known-group-event acquisition endpoint/item/byte limits, global credits, per-turn admission and yield delay, request deadline | 2 endpoints; 16 retained events and 128 KiB serialized events per endpoint; 2 global jobs; 1 admission/turn; 1 ms yield; 5 s request | resource, scheduler, input-recovery | Bounds the first owner-authorized exact-ID worker path while keeping capacity through admission. Production activation remains gated on the conforming SDK backend | Smaller budgets leave durable known-event demand retryable; larger budgets increase retained results or worker occupancy. EOSE never certifies unknown history | can affect acquired input and latency; not branch selection given equal retained input | Controlled two-relay worker admission, cancellation, stale-fence, saturation and ready-work tests; real backend conformance before activation |
...
| A11 | `runtime/account_worker/bounded_recovery.rs`, `runtime/account_worker.rs`; `bounded_known_group_acquisition_wait_keeps_worker_and_live_subscriptions_available`; `bounded_three_relay_scope_leaves_legacy_attempt_eligible`; `bounded_saturation_preserves_demand_then_partial_result_admits_known_event`; `bounded_stale_loss_and_route_results_cannot_clear_known_demand`; `bounded_shutdown_after_durable_prefix_survives_storage_reopen`; `retained_multi_epoch_backlog_advances_while_bounded_history_waits`; `bounded_executor_does_not_spend_an_unrelated_owner_selection` | Real SDK acquisition, generation fencing and two-relay conformance before production activation; broader group history remains under #1947 |
...
| 2026-09-23 | A11 bounded known-event worker preparation (#1947 P5) | Add a controlled exact-ID acquisition path with retained capacity through one-at-a-time admission. Preflight unsupported route shapes before reservation; keep production activation off until #1358 supplies a qualified backend. | Controlled worker tests cover two relays, three-relay decline, partial/saturated results, stale loss/route results, cancellation/reopen and retained epoch backlog; real SDK conformance pending |
