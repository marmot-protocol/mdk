...
| S2 | Protocol `convergence.md` “Applying the selected branch”; local canonicalization output contract | Tamarin application-output/invalidation lemmas; `engine_emits_only_canonical_branch_app_messages_after_convergence`; `rebuilt_engine_parks_losing_branch_app_for_revival_after_convergence` | M1.1 exact scenario-input/output ledger; M5.1 app projection adapter |
...
| R2 | Relay telemetry EOSE/backfill/reconciliation architecture; protocol requires equalized relevant history for equivalent selection | `stalled_epoch_backfill_still_arms_after_route_reconciliation`; `next_event_returns_when_a_live_delivery_arms_the_epoch_backfill`; retained-relay unequal-history reconciliation | M3.1 sustained unequal-history family |
...
| A10 | frozen-epoch reassessment interval, run continuation window, qualified local-observation threshold, and detector clock-skew allowance | 1 h / 24 h / 3 qualified observations / 5 min | input-recovery | Reassess existing demand without buying replay; report distinct paced local engine evaluations under current exhaustive scope and admission proof | Low intervals/thresholds over-report; high values delay reporting. Window bounds run projection. Owner retry survives reopen and clock correction independently. EOSE alone and timer ticks cannot mint samples | none — reporting changes neither branch choice nor completion; the owner alone authorizes acquisition | Qualified local evaluation, reorg, reopen, stale-evidence, clock and anti-spoofing tests |
| A11 | exact known-group-event acquisition endpoint/item/byte limits, global credits, per-turn admission, yield delay, probe interval and request deadline | 2 endpoints; 16 retained events and 128 KiB serialized events per endpoint; 2 global jobs; 1 admission/turn; 1 ms yield; 1 s probe; 5 s request | resource, scheduler, input-recovery | Bounds the first owner-authorized exact-ID worker path while keeping capacity through admission and avoiding per-delivery preparation writes during owner cooldown. Production activation remains gated on the conforming SDK backend | Smaller budgets leave durable known-event demand retryable; larger budgets increase retained results or worker occupancy. EOSE never certifies unknown history | can affect acquired input and latency; not branch selection given equal retained input | Controlled two-relay worker admission, cancellation, stale-fence, saturation and ready-work tests; real backend conformance before activation |
| A12 | `SDK_RECONCILIATION_MAX_ID_REQUESTS`, `SDK_RECONCILIATION_MAX_ITEMS_PER_ENDPOINT`, `SDK_RECONCILIATION_MAX_BYTES_PER_ENDPOINT`, `SDK_RECONCILIATION_MAX_SINGLE_EVENT_BYTES` | 16 exact-ID requests and 16 items; ordinary aggregate 128 KiB serialized event JSON, or one object up to 5 MiB in an otherwise empty pass; network cost charged by maximum endpoint per request | resource, input-recovery | Bounds the adapter's NIP-77 missing-ID acquisition and warm SDK cache returns under the existing two-second route deadline; the 5-MiB exception aligns with the pinned SDK's default normalized-message ceiling, not a protocol maximum; one-ID requests and the durable advisory cursor resume later IDs without treating fetched bytes as admitted | Lower values take more paced passes or leave a supported large event incomplete; higher values increase request cost and temporary retention. The first request can retain 5 MiB per endpoint before deduplication, plus one rejected boundary event; these limits are not wire-byte ceilings. A silent endpoint can still consume almost the whole deadline | can affect acquired input and latency; not branch selection given equal retained input | Two-relay 1,100-retained/eight-missing fixture, warm/mixed SDK cache and single-large-object cases, prefilter byte accounting, refused-ID rotation, duplicate item limit, fast withholding and silent endpoint tests, app comparison retry/pacing test; composed worker/cold-reopen validation remains #1947 |
| A13 | `MAX_COMPARISON_ENDPOINTS_PER_ROUTE`, `ONLINE_EPOCH_GAP_DRAIN_SLICE` | 4 already-admitted endpoints per frozen route; 40 ms owner scheduling slice between completed online EpochGap drain deliveries; the existing 4-route and 10-second acquisition quantum remain, with one additional 10-second owner-admission window for the whole returned pass; shared 2-credit process capacity remains | resource, scheduler, input-recovery | Admits a selected periodic-maintenance comparison or one selected Receive-seam EpochGap obligation to immutable off-worker SDK acquisition. For the EpochGap, the owner also schedules bounded queue admission and drain slices while serving safe commands; receive deadlines and recovery clocks span slices | A route with more endpoints uses the full inline executor. A slice yields only before the next receive; a single ingest or checkpoint can exceed 40 ms. SDK per-endpoint limits and this route cap do not bound total wire bytes or RSS | can affect latency and acquired input timing, not settled branch selection given equal retained input | Frozen-grant eligibility, stale fence/subscription rejection, held SDK request with worker command progress, online EpochGap admission and cancellation tests |
...
| E2, E3, E4, E8 | `message_processor/mod.rs`; `deferred_peel_retry_budget_refuses_without_terminal_dedup`; `deferred_peel_residence_survives_restart_and_backward_clock`; `peel_deferred_rows_capped_per_group_under_flood`; `peel_deferred_group_byte_budget_refuses_growth_without_persisting_it`; `peel_deferred_account_byte_budget_is_restart_safe`; `test_override_reproduces_256_row_capacity_refetch`; `deferred_peel_row_capacity_policy_sweep`; `resource_refusal_signals_immediately_once_per_epoch`; strict first-join chaos case; deferred retry/context tests | M3.1 fairness/restart/redelivery campaign |
...
| E13 | `message_processor/mod.rs`, `openmls_projection/resumable.rs`; `background_advance_bounds_work_across_sweeps_and_resumes_after_restart`; `background_advance_yields_on_elapsed_budget_without_abandoning_rows`; `resumable_search_matches_original_bfs_without_repeating_probes`; `resumable_slices_kill_with_live_state_restored_and_restart_from_durable_inputs` | Maintained `public_app_1024_message_backlog_recovers_completely`; wider device/relay responsiveness remains unverified |
...
| A7 | `marmot-app/lib.rs`; `cold_restart_reconciles_backlog_below_since_floor`; `stalled_epoch_backfill_still_arms_after_route_reconciliation` | M2.3 long-offline retained-relay matrix |
...
| A10 | `epoch_stall.rs`, `account_recovery/stall.rs`, `client/recovery.rs`; `qualified_stall_needs_three_distinct_paced_evaluations_without_replay`; `qualified_stall_certificate_and_interval_survive_reopen`; `qualified_stall_transaction_failure_and_clock_rebase_never_forge_samples`; `qualified_stall_rejects_new_loss_route_and_inventory_changes`; `qualified_local_observations_escalate_without_replay_and_survive_reorg`; `qualified_local_evidence_survives_reopen_without_replaying_or_reporting_twice`; `elapsed_time_and_unreadable_traffic_cannot_mint_qualified_samples`; `qualified_evidence_projection_resets_on_epoch_change_or_expired_run` | Production cohorts for the reporting interval; current SDK cannot certify exhaustive historical coverage, so unknown coverage remains uncounted |
| A11 | `runtime/account_worker/bounded_recovery.rs`, `runtime/account_worker.rs`; `bounded_known_group_acquisition_wait_keeps_worker_and_live_subscriptions_available`; `bounded_three_relay_scope_leaves_legacy_attempt_eligible`; `bounded_saturation_preserves_demand_then_partial_result_admits_known_event`; `bounded_stale_loss_and_route_results_cannot_clear_known_demand`; `bounded_shutdown_after_durable_prefix_preserves_pending_demand_on_reopen`; `bounded_shutdown_before_admission_reopens_unretained_demand`; `retained_multi_epoch_backlog_advances_while_bounded_history_waits`; `bounded_executor_does_not_spend_an_unrelated_owner_selection`; `bounded_grant_freeze_preserves_live_maintenance_observation`; `bounded_grant_keeps_installed_maintenance_boundary_checkpointable` | Real SDK acquisition, generation fencing and two-relay conformance before production activation; broader group history remains under #1947 |
| A12 | `transport-nostr-adapter/src/sdk_client.rs`; `large_retained_history_and_sparse_gap_use_finite_bounded_acquisition`; `warm_full_event_cache_resumes_under_combined_result_budget`; `mixed_cache_and_network_share_one_return_budget`; `cached_id_over_single_object_ceiling_keeps_smaller_id_reachable`; `one_large_network_event_is_recovered_within_single_object_ceiling`; `warm_large_event_waits_for_empty_next_pass_after_smaller_event`; `large_network_id_retries_after_a_smaller_cached_prefix`; `unadmitted_small_network_id_does_not_hide_deferred_large_id`; `duplicate_route_endpoint_preserves_cached_prefix_and_unique_obligations`; `large_network_result_survives_one_withholding_endpoint`; `duplicated_endpoint_item_limit_keeps_partial_event_and_incomplete_summary`; `silent_and_fast_withholding_endpoint_resume_two_ids_across_paced_passes`; `reconciliation_redelivers_seen_but_unretained_events_with_default_sdk_cache`; `unfinished_selected_id_suffix_stays_pending_and_owner_paced` | Composed account-worker admission, concurrent ordinary-replay bandwidth, process-crash and cold-reopen qualification remain under #1947/#1948 |
| A13 | `marmot-app/client/sync/comparison_job.rs`, `client/sync.rs`, `runtime/account_worker.rs`; `comparison_worker_held_sdk_request_keeps_status_command_ready`; `comparison_stale_result_preserves_cursor_and_debt`; `comparison_more_than_four_endpoints_keeps_inline_grant`; `comparison_credit_exhaustion_does_not_spend_reservation`; `explicit_catch_up_waits_for_held_comparison_before_revising_it`; `prebarrier_media_runs_before_deferred_explicit_catch_up`; `comparison_join_admits_owned_event_after_real_delivery_backpressure`; `comparison_join_timeout_keeps_cursor_and_retry_debt_after_real_queue_block`; `comparison_failed_queue_keeps_prepass_cursor_for_unqueued_suffix`; `online_epoch_gap_after_completed_startup_exposes_owner_wait` | Startup and explicit catch-up remain inline; Receive offload is limited to one selected EpochGap and does not run convergence during that grant. Broader resumption and process-crash qualification remain under #1948 |

E3 recalibration (2026-09-06, PR #1711): raise the raw-row cap from 512 to 2,048 to reduce transport-refused
input during large catch-up. The 16/64 MiB byte ceilings and 32-context retry limit are unchanged. The cap alone
did not repair full-app recovery; the dedicated 1,024-message public job also exercises generation barriers,
projection and relay reconciliation. This does not establish that 2,048 is an optimal cap.
...
row without terminal deduplication, while E3's row and encoded-byte limits return typed `ResourceRefused`; exact-ID regression coverage proves both
...
| `PeelDeferred` transport | Changed peel context retries it; row/group-byte/account-byte capacity, retry, or residence exhaustion emits typed `resource_refused`, releases or refuses the row, and preserves exact-ID redelivery | `deferred_peel_retries_after_epoch_advance`; `deferred_peel_retry_budget_refuses_without_terminal_dedup`; `deferred_peel_residence_survives_restart_and_backward_clock`; `peel_deferred_rows_capped_per_group_under_flood`; `peel_deferred_group_byte_budget_refuses_growth_without_persisting_it`; `peel_deferred_account_byte_budget_is_restart_safe` |
...
- [ ] Promote the same-source-epoch cross-route topology exposed during #1236 and repaired at engine level by #1285
  ...
  - [ ] Expand the core vector into process, container, and VM executions, with delivery permutations and
...
5. [ ] Complete the #1285 cross-route regression across every capable adapter. The `CgkaEngine` checkpoint is now permanent
   ...
   through retained relay histories; app-runtime and isolated-process public-projection evidence is now complete, and
   the scheduled container checkpoint consumes the same public scenario/oracle while retaining its distributed
   artifacts. VM execution and engine-private evidence unavailable through those public adapters remain open.
   ...
   #1433 and #1435, do not inherit that result. The scheduled container checkpoint executes the current source revision
   and retains its exact normalized manifest and report; every future evidence claim must continue to name its tested
   revision rather than carrying an older green soak forward.
...
| 2026-07-30 | L1 epoch-stall backfill escalation | Closed the open self-heal loop for repeated epoch-gap backfills: three arms in one unrecovered run now report a typed `EpochStallEscalated` runtime event plus a durable `epoch_stall_backfill_escalated` audit row naming the app-owned stronger repair, latched once per run and reset when the device is observed leaving an epoch it did not arm at (ledger A9) | `cargo test -p marmot-app client::epoch_stall --lib`; `cargo test -p marmot-app --test epoch_stall_backfill_audit`; `cargo test -p marmot-forensics`; `just convergence-ledger-gate` |
...
| 2026-08-06 | L2 terminal disposition for accepted sends | Closed the "terminated with an explicit outcome" half of L2: an accepted send now reports whether it published or is retained, a halted group refuses new sends with a typed error instead of engine-bug error text, and the two transitions that permanently discard a retained intent (disband, local copy removed) withdraw the group's unpublished sends so they stop reporting as pending. The pending-to-delivered flip also became a distinct delivery-state signal on both the publish and the explicit-retry path. The "released, regenerated" half still awaits M3.1 crash/failure workloads, so L2 stays `partially-covered` | [MDK #1426](https://github.com/marmot-protocol/mdk/pull/1426); `cargo test -p storage-sqlite`; `cargo test -p marmot-app --lib`; `cargo test -p cgka-engine --features test-policy-overrides`; `cargo test -p marmot-uniffi`; `just fast-ci` |
...
| 2026-08-27 | L1 frozen-epoch escalation evidence | Closed the epoch-stall detector's last self-heal blind spot: a group wedged at one stalled epoch could arm exactly once, because every later arm needed an epoch advance the missing commit was the only source of. Same-epoch re-arms are now paced on a wall clock and escalation for that shape counts relay-confirmed evidence — end-of-stored-events replay completions that recovered nothing — rather than arms, so minted undecryptable traffic can pace attempts but cannot manufacture a report. Arms an unrelated stall weeks later no longer join a long-dead run. The evidence and its wall-clock arm mark persist per group so restarts neither erase the count nor become the re-arm clock (ledger A10) | `cargo nextest run -p marmot-app --features test-policy-overrides,cgka-engine/test-crash-hooks`; `cargo test -p storage-sqlite`; `just convergence-ledger-gate`; `cargo test -p cgka-conformance-simulator --test protocol_decision_gate` |
| 2026-09-04 | E3 deferred-peel capacity calibration | Raised the per-group row limit from 256 to 512 after the strict 384-message retained-history case showed the old bound caused a redundant full-history refetch; added exact encoded-byte limits of 16 MiB per group and 64 MiB per account, restart-safe durable usage reconstruction, aggregate refusal/high-water metrics, and feature-gated row/byte policy overrides. The 256/512/1,024 sweep measured 107/0/0 row refusals and 2/1/1 full-history syncs; both larger limits peaked at 363 rows and 379,456 encoded bytes. The public refusal outcome and exact-ID redelivery contract remain unchanged. | `peel_deferred_rows_capped_per_group_under_flood`; `peel_deferred_group_byte_budget_refuses_growth_without_persisting_it`; `peel_deferred_account_byte_budget_is_restart_safe`; `large_natural_history_fits_raised_deferred_capacity`; `test_override_reproduces_256_row_capacity_refetch`; ignored `deferred_peel_row_capacity_policy_sweep`; constant/protocol gates |
| 2026-09-06 | E13 bounded background recovery | Share the row allowance across the background loop and cooperatively yield after 500 ms; materialize historical peel contexts once per sweep and restore live storage before async peeling. Preserve the durable generation barrier and key retention. The public 1,024-message regression passes twice, including fresh traffic and restart. | Three deferred-peel regressions; `target/background-recovery-20260906/`; local app runs only, no device validation |
| 2026-09-12 | E13 resumable candidate reconstruction | Add a 32-probe slice ceiling inside the existing cooperative background deadline. Preserve breadth-first order, cumulative replay limits, full-result selection, and live-state restoration. Exact input/state identity controls memory-only continuation reuse; SQLCipher fingerprints canonical and retained replay state so unrelated app-connection commits cannot starve progress. Restart discards scratch progress. | Differential selection/peel tests, frozen-membership and generation invalidation, encrypted process-kill recovery; isolated measurement and unchanged 50-member app canary recorded in `target/app-resumable-replay-20260912-1/` |
...
| 2026-09-23 | A10 recovery owner integration (#1946) | Preserve numeric constants; replace replay/EOSE counting with distinct paced local evaluations under qualified scope/admission proof. Detector time signals only reassess; account retry owns network authority. Retain reorg, clock, reopen, refusal and stale-evidence guarantees. | Focused storage/owner/runtime tests recorded in `further-context/account-recovery-integration.md`; final policy gates pending |
| 2026-09-23 | A11 bounded known-event worker preparation (#1947 P5) | Add a controlled exact-ID acquisition path with retained capacity through one-at-a-time admission. Preflight unsupported route shapes before reservation, preserve live maintenance observations and probe on a bounded interval; keep production activation off until #1358 supplies a qualified backend. | Controlled worker tests cover two relays, three-relay decline, partial/saturated results, stale loss/route results, cancellation/reopen, cooldown burst and retained epoch backlog; real SDK conformance pending |
| 2026-09-25 | A12 selective missing-ID acquisition budget (#1947 P6) | Replace the adapter's unbounded 128-ID SDK fetch with request-local one-ID acquisitions under a 16-request, 16-item and ordinary 128-KiB-serialized-event combined return allowance inside the existing two-second comparison deadline. Charge warm SDK cache hits, allow one 5-MiB-or-smaller object in an empty pass, and continue after fast endpoint-local failures without clearing their obligations. Preserve partial owned events and classify endpoint budget/deadline exits as incomplete. Production bounded recovery remains off. | Baseline two-relay 1,100-retained/eight-missing fixture sent 331,112 prefilter EVENT JSON bytes from one relay in one pass; the ordinary 40-KiB fixture keeps each pass below its 180-KiB per-relay event-byte ceiling and resumes all eight IDs within four passes. Warm/mixed cache, single-large-object, over-ceiling, mixed endpoint, and app retry/pacing tests pass; broader ordinary replay and worker SQLCipher admission are unproven by these tests. |
| 2026-09-25 | A13 periodic comparison worker resume (#1947 P7) | Move the selected maintenance comparison's bounded SDK wait to an owned task after worker activation; keep its memory-only cursor and event batch unadmitted until the worker rechecks the grant and subscription attempt. Reuse two global recovery credits and cap offloaded routes at four endpoints each. | Focused app tests cover grant fallback, stale-result rejection, retry cursor and held-request worker progress; startup, explicit, receive, activation and drain waits remain inline. |
| 2026-09-26 | A13 steady-state Receive EpochGap continuation | Reuse the selected grant and shared credit for one eligible EpochGap after startup. Keep activation, admission, drain, checkpoint and terminal on the account owner; let the immutable adapter request and bounded queue submission wait off-owner. Slice the owner drain at 40 ms only between completed deliveries, retaining its receive and recovery deadlines. | A two-relay live-worker fixture holds the selected adapter reconciliation future while status, healthy-route send and healthy-route live delivery finish within their bounds. Its missing commit was durably buffered by ordinary receive, skipped as a duplicate in the selected drain, and advanced by scheduled local convergence after that attempt's terminal and before a later network retry. This does not show that the selected attempt itself qualified the gap; broader restart and fault matrices remain open. |
