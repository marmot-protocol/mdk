...
| A12 | `SDK_RECONCILIATION_MAX_ID_REQUESTS`, `SDK_RECONCILIATION_MAX_ITEMS_PER_ENDPOINT`, `SDK_RECONCILIATION_MAX_BYTES_PER_ENDPOINT` | 16 exact-ID requests, 16 items and 128 KiB serialized event JSON in the combined returned batch; network cost charged by maximum endpoint per request | resource, input-recovery | Bounds the adapter's NIP-77 missing-ID acquisition and warm SDK cache returns under the existing two-second route deadline; one-ID requests and the durable advisory cursor resume later IDs without treating fetched bytes as admitted | Lower values take more paced passes or leave oversized objects incomplete; higher values increase request cost and retained memory. One rejected boundary event can exceed the stated received budget, and the limit is not a wire-byte ceiling | can affect acquired input and latency; not branch selection given equal retained input | Two-relay 1,100-retained/eight-missing fixture, warm/mixed SDK cache budgets, prefilter byte accounting, refused-ID rotation, duplicate item limit and mixed endpoint deadline tests; composed worker/cold-reopen validation remains #1947 |
...
| A12 | `transport-nostr-adapter/src/sdk_client.rs`; `large_retained_history_and_sparse_gap_use_finite_bounded_acquisition`; `warm_full_event_cache_resumes_under_combined_result_budget`; `mixed_cache_and_network_share_one_return_budget`; `oversized_cached_id_keeps_later_cached_id_reachable`; `duplicated_endpoint_item_limit_keeps_partial_event_and_incomplete_summary`; `silent_endpoint_deadline_keeps_healthy_partial_event_and_incomplete_summary`; `reconciliation_redelivers_seen_but_unretained_events_with_default_sdk_cache` | Composed account-worker admission, concurrent ordinary-replay bandwidth, process-crash and cold-reopen qualification remain under #1947/#1948 |
...
| 2026-09-25 | A12 selective missing-ID acquisition budget (#1947 P6) | Replace the adapter's unbounded 128-ID SDK fetch with request-local one-ID acquisitions under a 16-request, 16-item and 128-KiB-serialized-event combined return allowance inside the existing two-second comparison deadline. Charge warm SDK cache hits against the same allowance. Preserve partial owned events and classify endpoint budget/deadline exits as incomplete. Production bounded recovery remains off. | Baseline two-relay 1,100-retained/eight-missing fixture sent 331,112 prefilter EVENT JSON bytes from one relay in one pass; the new fixture keeps each pass below its 180-KiB per-relay event-byte ceiling and resumes all eight IDs within four passes. Warm/mixed cache, oversized cached event and mixed item-limit/deadline tests pass; broader ordinary replay and worker admission are unproven by this adapter test. |
