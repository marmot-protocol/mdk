...
The `just adversarial-reliability-ci` gate executes every catalog family (including the feature-gated witness and replay-budget
...
- [x] Release hardening produces a reviewed convergence evidence bundle that lists covered decision routes, models,
...
| 2026-08-05 | Milestone 6 review hardening | Made candidate-win fork displacement transactional while preserving non-descendant dispositions and queued work, while leaving its in-flight candidate owned by the ingest retry; bounded candidate-context construction across a complete drain; documented finite-input fairness and legacy-stamp fail-closed behavior; scoped route claims to adapter capabilities; made offline exact-state refusal explicit; and hardened distributed timeout, proxy-listen, and contention-duration contracts | `candidate_win_restores_non_descendant_states_and_queued_work`; `inbound_apply_record_mirror_failure_does_not_split_epoch_state`; `pairwise_candidate_win_leaves_old_incumbent_reconsiderable`; `candidate_context_materialization_is_bounded_per_drain`; `pass_opens_while_app_message_intents_are_queued`; `legacy_serialized_stamp_is_not_treated_as_anchor_verified`; `legacy_own_commit_stamp_fails_closed_without_partial_reorg`; `route_equivalence`; `distributed_runner` |
...
- [x] Confirm the long-lived adversarial reliability test source uses a capability-based filename.
...
- [x] Run the full affected simulator, app-runtime, process, distributed, formal, and repository fast-CI gates after
  the renames.
