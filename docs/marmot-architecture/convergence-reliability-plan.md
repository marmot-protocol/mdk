...
| A10 | frozen-epoch re-arm pacing, run continuation window, and fruitless-completion threshold | 1 h / 24 h / 3 confirmed fruitless completions | input-recovery | Lets a group wedged at one stalled epoch be reported: paces its same-epoch re-arms on a wall clock, bounds how far apart two arms can be and still be one run, and escalates on that many end-of-stored-events replay completions that recovered nothing | Interval too low costs extra whole-account replays and too high delays the report; window too low splits one field stall into several runs and too high revives a dead one; completion threshold too low reports a group a later replay would have healed, too high leaves a wedged device silent, the 2026-08 field failure. Interval must stay below the window | none — it reports, and the paced re-arm changes only when a replay already permitted by A6 runs | Frozen-epoch detector-driven escalation and restart-persistence tests, pacing/latch/reset unit tests, additional production cohorts |
