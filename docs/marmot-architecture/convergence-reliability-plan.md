...
| A11 | `runtime/account_worker/bounded_recovery.rs`, `runtime/account_worker.rs`; `bounded_known_group_acquisition_wait_keeps_worker_and_live_subscriptions_available`; `bounded_executor_does_not_spend_an_unrelated_owner_selection` | Real SDK acquisition and two-relay conformance before production activation; broader group history remains under #1947 |
...
| 2026-09-23 | A11 bounded known-event worker preparation (#1947 P5) | Add a controlled exact-ID acquisition path with retained capacity through one-at-a-time admission. Keep production activation off until #1358 supplies a qualified backend. | `bounded_known_group_acquisition_wait_keeps_worker_and_live_subscriptions_available`; `bounded_executor_does_not_spend_an_unrelated_owner_selection`; real SDK and wider P5 acceptance pending |
