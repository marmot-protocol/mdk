...
| A13 | `MAX_COMPARISON_ENDPOINTS_PER_ROUTE` | 4 already-admitted endpoints per frozen route; the existing 4-route and 10-second acquisition quantum remain, with one additional 10-second owner-admission window for the whole returned pass; shared 2-credit process capacity remains | resource, scheduler, input-recovery | Admits only a selected periodic-maintenance comparison to an immutable off-worker SDK task; the owner retains activation, cursor persistence, event queue, drain and settlement | A route with more endpoints uses the full inline executor. SDK per-endpoint limits and this route cap do not bound total wire bytes or RSS | can affect latency and acquired input timing, not settled branch selection given equal retained input | Frozen-grant eligibility, stale fence/subscription rejection, held SDK request with worker command progress, admission and cancellation tests |
