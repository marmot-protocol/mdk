...
| A12 | `SDK_RECONCILIATION_MAX_ID_REQUESTS`, `SDK_RECONCILIATION_MAX_ITEMS_PER_ENDPOINT`, `SDK_RECONCILIATION_MAX_BYTES_PER_ENDPOINT` | 16 exact-ID requests, 16 received items and 128 KiB serialized event JSON per endpoint per comparison | resource, input-recovery | Bounds the adapter's NIP-77 missing-ID acquisition under the existing two-second route deadline; one-ID requests and the durable advisory cursor resume later IDs without treating fetched bytes as admitted | Lower values take more paced passes or leave oversized objects incomplete; higher values increase request cost and retained memory. One rejected boundary event can exceed the stated received budget, and the limit is not a wire-byte ceiling | can affect acquired input and latency; not branch selection given equal retained input | Two-relay 1,100-retained/eight-missing fixture, prefilter byte accounting, refused-ID rotation, duplicate item limit and mixed endpoint deadline tests; composed worker/cold-reopen validation remains #1947 |
...
| 2026-09-25 | A12 selective missing-ID acquisition budget (#1947 P6) | Replace the adapter's unbounded 128-ID SDK fetch with request-local one-ID acquisitions under a 16-request, 16-received-item and 128-KiB-serialized-event per-endpoint allowance inside the existing two-second comparison deadline. Preserve partial owned events and classify endpoint budget/deadline exits as incomplete. Production bounded recovery remains off. | Baseline two-relay 1,100-retained/eight-missing fixture sent 331,112 prefilter EVENT JSON bytes from one relay in one pass; the new fixture keeps each pass below its 180-KiB per-relay event-byte ceiling and resumes all eight IDs within four passes. Mixed item-limit/deadline outcomes and the adapter suite pass; broader ordinary replay and worker admission are unproven by this adapter test. |
