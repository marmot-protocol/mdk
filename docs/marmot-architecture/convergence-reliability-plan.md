...
| E11 | `FOREGROUND_DEFERRED_PEEL_BUDGET_MS` | 250 ms | resource, scheduler | Gives current-context deferred input one bounded opportunity before preparing new outbound wire bytes | Exhaustion durably queues the original intent as `AcceptedPending`; no wire bytes are prepared; elapsed work and overrun are observable | intended no | Notify-gated 47/64-row tests, zero-wire assertion, monotonic timing metrics, and benchmark matrix |
| E12 | `MAX_FOREGROUND_DEFERRED_ROWS` | 4 rows | resource, scheduler | Bounds synchronous deferred-row attempts per outbound preflight independently of peeler speed | Unattempted current-context rows remain durable and resume from the oldest uncompleted row after restart | intended no | Four-row boundary, restart-resume, same-fingerprint fairness, and 47/64-row backlog tests |
