...
| `account_open` | One sample per newly spawned account worker, from worker spawn until the ready signal. | Covers SQLCipher/session hydration, local engine-group reconciliation, and local projection backfill. The read-snapshot capture runs after the ready signal (mdk#1161) and is measured separately. It does not include relay work. |
