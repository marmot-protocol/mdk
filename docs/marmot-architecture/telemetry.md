...
| `account_relay_drain` | One `sync_sdk_relay` pass from the start of account-wide delivery draining through its checkpoint during a telemetry-carrying startup sync. | Separates startup relay receive/ingest/checkpoint work from transport activation, subscription registration, overflow recovery, and the remaining `account_sync` envelope. Explicit catch-up, maintenance catch-up, and `backfill_sdk_relay` drains do not emit this sample. |
...
| `outbound_message_queue_wait` | Time from enqueueing `SendMessage` or `SendAppEvent` until the account worker begins handling it. | Isolates delay behind startup sync, explicit catch-up, convergence maintenance, or another worker command. A sample exists only after dispatch, so attempts can trail `outbound_message_total_caller_latency`; recorded samples are always successful, making failures structurally zero. |
