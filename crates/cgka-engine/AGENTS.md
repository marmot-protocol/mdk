...
- **You want to...:** Understand fork handling
  - **Open...:** the convergence admission gate (`commit_should_enter_convergence`) in
    ...
    distributed convergence for every member; the missing-anchor fail-closed halt lives in the `WrongEpoch` branch
...
checkpoints (#1285); a missing in-horizon anchor fails closed loudly (durable `Unrecoverable` +
`GroupEvent::GroupUnrecoverable`). The former pairwise fast-path (`ForkRecoveryManager`, `committed_from` routing,
