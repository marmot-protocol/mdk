...
- **A commit row's `epoch` column is the epoch that commit forks FROM, on every inbound door.** The convergence door
  stamps the projected `source_epoch`; direct ingest stamps the commit's own wire epoch, which is always below
