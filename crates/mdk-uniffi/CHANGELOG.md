...
- MIP-04 encrypted media support: five new methods on `Mdk` — `encrypt_media_for_upload`, `encrypt_media_for_upload_with_options`, `decrypt_media_from_download`, `create_media_imeta_tag`, and `parse_media_imeta_tag` — plus three new UniFFI records: `EncryptedMediaUploadResult`, `MediaReferenceRecord`, and `MediaProcessingOptionsInput`. The `mip04` feature is now always active in the UniFFI crate. ([#209](https://github.com/marmot-protocol/mdk/pull/209))
