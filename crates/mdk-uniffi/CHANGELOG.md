...
- Added `disappearing_message_secs: Option<u64>` field to the `Group` UniFFI record and `disappearing_message_secs: Option<Option<u64>>` to the `GroupDataUpdate` record for configuring disappearing messages via foreign language bindings. ([#253](https://github.com/marmot-protocol/mdk/pull/253))
