...
impl From<group_types::Group> for Group {
    fn from(g: group_types::Group) -> Self {
        Self {
            ...
            image_key: g.image_key.map(|k| k.into_inner().to_vec()),
            image_nonce: g.image_nonce.map(|n| n.into_inner().to_vec()),
...
impl From<welcome_types::Welcome> for Welcome {
    fn from(w: welcome_types::Welcome) -> Self {
        ...
        Self {
            ...
            group_image_key: w.group_image_key.map(|k| k.into_inner().to_vec()),
            group_image_nonce: w.group_image_nonce.map(|n| n.into_inner().to_vec()),
...
pub fn prepare_group_image_for_upload(
    ...
    Ok(GroupImageUpload {
        encrypted_data: prepared.encrypted_data.into_inner(),
        encrypted_hash: prepared.encrypted_hash.to_vec(),
        image_key: prepared.image_key.into_inner().to_vec(),
        image_nonce: prepared.image_nonce.into_inner().to_vec(),
