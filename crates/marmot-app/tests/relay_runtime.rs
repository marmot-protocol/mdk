...
async fn app_runtime_readd_after_remove_resurfaces_removed_member_group() {
    ...
    assert!(!accepted.pending_confirmation);
    assert!(!accepted.archived);
    let duplicate_accept = accept_group_invite_retrying_busy(&runtime, &bob_id, &group_id)
        .await
        .expect_err("an already accepted member projection is not a pending invite");
    assert!(matches!(duplicate_accept, AppError::GroupInviteNotPending));
    ...
    runtime
        .set_group_archived(&bob_id, &group_id_hex, true)
        .await
        .expect("the terminal projection can remain archived while a stale invite action exists");
    let removed = app.group(&bob_label, &group_id_hex).unwrap().unwrap();
    assert_eq!(removed.self_membership, SelfMembership::Removed);
    assert!(
    ...
    let runtime = MarmotAppRuntime::new(app.clone());
    let restart_accept = accept_group_invite_retrying_busy(&runtime, &bob_id, &group_id)
        .await
        .expect_err("restart must not make a terminal stale invite acceptable");
    assert!(matches!(restart_accept, AppError::GroupInviteNotPending));
    runtime
        .publish_key_package(&bob_id)
        .await
        .expect("restart must publish a fresh key package before a genuine re-invite");
    let mut events = runtime.subscribe();
    ...
    let restarted = runtime
        .group_roster(&bob_id, &group_id)
        .await
        .expect("accepted re-invite must survive account-worker restart");
    assert_eq!(restarted.self_membership, SelfMembership::Member);
    let restarted_projection = app.group(&bob_label, &group_id_hex).unwrap().unwrap();
    assert!(!restarted_projection.pending_confirmation);
    assert!(!restarted_projection.archived);
    runtime.shutdown().await;
}
...
async fn relay_app_runtime_projects_typed_reactions_and_deletes() {
    ...
    alice
        .delete_message(&group_id, &target_message_id)
        .await
        .unwrap();
...
async fn live_report_indicator_uses_existing_timeline_subscription() {
    ...
    let alice = create_network_ready_identity(&runtime, setup().relay_options_only()).await;
    let bob = create_network_ready_identity(&runtime, setup()).await;
    let a = alice.account.account_id_hex;
    let b = bob.account.account_id_hex;
    let mut events = runtime.subscribe();
    let group = runtime
        .create_group(&a, "reports", std::slice::from_ref(&b), None)
        .await
        .unwrap();
    wait_for_event(&mut events,|e|matches!(e,MarmotAppEvent::GroupJoined{account_id_hex,group_id,..} if account_id_hex==&b && group_id==&group)).await;
    accept_group_invite_retrying_busy(&runtime, &b, &group)
        .await
        .unwrap();
    let sent = runtime
        .send_message(&b, &group, b"review this".to_vec())
        .await
        .unwrap();
    let target = &sent.message_ids[0];
    ...
    .await
    .unwrap();
    let after = runtime.chat_list_row(&a, &group_hex).unwrap().unwrap();
...
async fn offline_member_recovers_dismissal_label_after_admin_demotion() {
    ...
    let apps: Vec<_> = dirs
        ...
        .collect();
    let alice = MarmotAppRuntime::new(apps[0].clone());
    let bob = MarmotAppRuntime::new(apps[1].clone());
    let carol = MarmotAppRuntime::new(apps[2].clone());
    let setup = || AccountSetupRequest {
        default_relays: vec![endpoint(&url)],
        bootstrap_relays: vec![endpoint(&url)],
        publish_initial_key_package: true,
    ...
    let a = create_network_ready_identity(&alice, setup())
        .await
        .account
        .account_id_hex;
    let b = create_network_ready_identity(&bob, setup())
        .await
        .account
        .account_id_hex;
    let c = create_network_ready_identity(&carol, setup())
        .await
        .account
        .account_id_hex;
    ...
    let group = alice
        .create_group(&a, "review room", &[b.clone(), c.clone()], None)
        .await
        .unwrap();
    wait_for_event(
        &mut bob_events,
    ...
    .await;
    accept_group_invite_retrying_busy(&bob, &b, &group)
        .await
        .unwrap();
    accept_group_invite_retrying_busy(&carol, &c, &group)
        .await
        .unwrap();
    carol.shutdown_and_close().await.unwrap();
    drop(carol);
    alice.promote_admin(&a, &group, &b).await.unwrap();
    timeout(Duration::from_secs(30), async {
        loop {
            if bob.group_mls_state(&b, &group).await.unwrap().epoch >= 2 {
                break;
            }
            sleep(Duration::from_millis(50)).await;
        }
    ...
    .unwrap();
    let sent = bob
        .send_message(&b, &group, b"reported while carol is offline".to_vec())
        .await
        .unwrap();
    let target = &sent.message_ids[0];
    let report = bob
    ...
    .await
    .unwrap();
    alice.demote_admin(&a, &group, &b).await.unwrap();
    timeout(Duration::from_secs(30), async {
        loop {
            if bob.group_mls_state(&b, &group).await.unwrap().epoch >= 3 {
                break;
            }
            sleep(Duration::from_millis(50)).await;
        }
    ...
    .await
    .unwrap();
    let reopened_carol_app = MarmotApp::with_relay_and_config(
        dirs[2].path(),
        url.clone(),
        MarmotAppConfig::default().with_allow_loopback_relay_endpoints(true),
    );
    let carol = MarmotAppRuntime::new(reopened_carol_app.clone());
    carol.reconcile_accounts().await.unwrap();
