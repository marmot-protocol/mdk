...
- Preserve normalized line breaks in ingested kind:0 `about` text while still removing
  ...
  its line breaks is repaired when the same event is fetched again, except for accounts
  held on this device, which recover on their next profile publish. (#1973)
