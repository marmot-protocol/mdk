...
impl PermissionLease {
    pub(crate) fn with_permission<T>(&self, work: impl FnOnce() -> T) -> Option<T> {
        let map = self.permissions.0.lock().ok()?;
        let p = map.get(&self.store)?;
        (p.generation == self.generation && p.categories.allows(self.category)).then(work)
    }
    pub(crate) fn allowed(&self) -> bool {
        self.permissions.0.lock().is_ok_and(|map| {
            map.get(&self.store).is_some_and(|p| {
                p.generation == self.generation && p.categories.allows(self.category)
            })
        })
    }
}
impl Permissions {
    pub(super) fn forget_account(&self, account_id: &str) {
        self.0
            ...
            .retain(|_, p| p.account_id != account_id);
    }
    ...
    fn ensure_host_managed_attachments(&self) -> Result<(), AppError> {
            ...
            != AttachmentAcquisitionMode::HostManaged
        {
            return Err(AppError::InvalidEncryptedMedia(
                "host-managed attachment mode required".to_owned(),
            ));
        }
        Ok(())
    }
    ...
    pub async fn begin_attachment_permission_update(
        ...
        let result = self
            .attachment_read(account_ref, move |storage, _| {
                ...
                let mut map = permissions
                    .0
                    .lock()
                    .map_err(|_| AppError::RuntimeStopping)?;
                let account = app.account_home().account(&account_reference)?;
                if account.signed_out {
                    return Err(AppError::InvalidEncryptedMedia(
                        "automatic attachment approval requires a signed-in account".to_owned(),
                    ));
                }
                map.insert(
                    store,
            ...
            .attachment_read(account_ref, move |storage, loopback| {
                ...
                let allowed = config.cursor_persistence != CursorPersistence::Frozen
                    && (config.attachment_acquisition_mode
                        ...
                        || map.get(&identity).is_some_and(|p| {
                            p.categories
                                .allows(super::attachment_history::category(&reference.media_type))
                        }));
