...
-- Create a composite index for the new sort order (created_at DESC, id DESC)
-- This ensures deterministic ordering even when timestamps are equal (e.g., same-second messages)
CREATE INDEX IF NOT EXISTS idx_messages_sorting ON messages(mls_group_id, created_at DESC, id DESC);
