...
| `pairwise_fork_recovery` | `message_processor/ingest.rs`, `fork_recovery.rs` | partial: bounded abstract route lifecycle | partial: sibling-model pairwise-loser terminalization | `cross-route-own-commit-recovery/v1`, `pairwise_incumbent_defers_to_deeper_convergence_branch`, and `pairwise_candidate_win_leaves_old_incumbent_reconsiderable` | Pairwise displacement and deeper-branch reconsideration are covered in-process; retained-history and external-adapter permutations remain open. |
...
| `application_disposition` | `message_processor/ingest.rs`, `openmls_projection.rs` | covered: reference dispositions | covered: output invalidation | scenario-input ledgers and bidirectional decryptability probes, including `cross-route-own-commit-recovery/v1` | Sender-visible application invalidation during branch replacement remains incomplete. |
