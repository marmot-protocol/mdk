...
- No Nostr SDK key management. The caller supplies the signer/decrypter that
  already belongs to the account-device identity.
...
publishes final kind `445` group events. Welcome outputs are already signed
NIP-59 gift wraps because NIP-59 construction needs the local signer.
