#!/usr/bin/env bash
# Native media-diagnostic DTO lift/lower round trips. This does not package Apple/Android artifacts.
set -euo pipefail
cd "$(dirname "$0")/../.."
...
case "$smoke_language" in
  ...
  kotlin)
    ...
    ;;
  *) echo 'Usage: media-diagnostics-smoke.sh swift|kotlin' >&2; exit 2 ;;
esac
CARGO_BUILD_JOBS="${CARGO_BUILD_JOBS:-2}" cargo build -p marmot-uniffi --features cli --locked
...
if [[ "$smoke_language" == swift ]]; then
  cat "$smoke_dir/marmot_uniffi.swift" crates/marmot-uniffi/tests/media_diagnostics_smoke.swift > "$smoke_dir/CombinedSmoke.swift"
  ...
  kotlinc "$smoke_dir/dev/ipf/marmotkit/marmot_uniffi.kt" crates/marmot-uniffi/tests/media_diagnostics_smoke.kt \
  ...
  java -Djna.library.path="$smoke_target" -cp "$smoke_dir/smoke.jar:$MDK_KOTLIN_CLASSPATH" \
    dev.ipf.marmotkit.Media_diagnostics_smokeKt
fi
