...
case "$smoke_language" in
  ...
  kotlin)
    ...
    ;;
  *) echo 'Usage: chat-projections-smoke.sh swift|kotlin' >&2; exit 2 ;;
esac
...
if [[ "$smoke_language" == swift ]]; then
  cat "$smoke_dir/marmot_uniffi.swift" crates/marmot-uniffi/tests/chat_projections_smoke.swift > "$smoke_dir/CombinedSmoke.swift"
  ...
  kotlinc "$smoke_dir/dev/ipf/marmotkit/marmot_uniffi.kt" crates/marmot-uniffi/tests/chat_projections_smoke.kt \
  ...
  java -Djna.library.path="$smoke_target" -cp "$smoke_dir/smoke.jar:$MDK_KOTLIN_CLASSPATH" \
    dev.ipf.marmotkit.Chat_projections_smokeKt
fi
