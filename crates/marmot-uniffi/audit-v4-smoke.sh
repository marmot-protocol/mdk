#!/usr/bin/env bash
# Native audit v4 metadata DTO lift/lower round trips. This does not package Apple/Android artifacts.
set -euo pipefail
cd "$(dirname "$0")/../.."
...
case "$smoke_language" in
  ...
  kotlin)
    ...
    ;;
  *) echo 'Usage: audit-v4-smoke.sh swift|kotlin' >&2; exit 2 ;;
esac
...
if [[ "$smoke_language" == swift ]]; then
  cat "$smoke_dir/marmot_uniffi.swift" crates/marmot-uniffi/tests/audit_v4_smoke.swift > "$smoke_dir/CombinedSmoke.swift"
  ...
  kotlinc "$smoke_dir/dev/ipf/marmotkit/marmot_uniffi.kt" crates/marmot-uniffi/tests/audit_v4_smoke.kt \
  ...
  java -Djna.library.path="$smoke_target" -cp "$smoke_dir/smoke.jar:$MDK_KOTLIN_CLASSPATH" \
    dev.ipf.marmotkit.Audit_v4_smokeKt
fi
