#!/usr/bin/env bash
# Native DTO round trips; this does not exercise a host consent screen or staging.
set -euo pipefail
cd "$(dirname "$0")/../.."
...
case "$smoke_language" in
  ...
  kotlin)
    ...
    ;;
  *) echo 'Usage: usage-diagnostics-smoke.sh swift|kotlin' >&2; exit 2 ;;
esac
cargo build -p marmot-uniffi --features cli,otlp-export,product-analytics-export --locked
...
if [[ "$smoke_language" == swift ]]; then
  # UniFFI's converter helpers are fileprivate, so concatenate the fixture.
  cat "$smoke_dir/marmot_uniffi.swift" crates/marmot-uniffi/tests/usage_diagnostics_smoke.swift > "$smoke_dir/CombinedSmoke.swift"
  ...
  kotlinc "$smoke_dir/dev/ipf/marmotkit/marmot_uniffi.kt" crates/marmot-uniffi/tests/usage_diagnostics_smoke.kt \
  ...
  java -Djna.library.path="$smoke_target" -cp "$smoke_dir/smoke.jar:$MDK_KOTLIN_CLASSPATH" \
    dev.ipf.marmotkit.Usage_diagnostics_smokeKt
fi
