...
- Keep one spelling per product capability: `keys`, `chats`, top-level `--account`, and positional basics for common
  group and message flows. Do not add hidden compatibility aliases for old command shapes.
- Keep daemon behavior real. `dm daemon start|stop|status`, the `dmd` binary, and socket-backed execution should be
