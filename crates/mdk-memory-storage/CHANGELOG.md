...
- Replaced 6 identical MLS storage struct definitions and their `Debug`/`new`/`clone_data`/`restore_data` impls with `mls_store_base!` and `mls_single_key_ops!` macros, reducing `mls_storage/mod.rs` by ~400 lines.
