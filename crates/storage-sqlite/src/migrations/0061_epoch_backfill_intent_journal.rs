//! Migration 0061: durable per-account epoch-backfill intent state.
...
mod tests {
    ...
    #[test]
    fn migration_adds_an_empty_singleton_blob_journal_after_schema_60() {
        ...
        run(&mut connection, &MIGRATIONS[..61]).unwrap();
    ...
    #[test]
    fn schema_60_runner_refuses_a_journal_database_without_mutating_the_blob() {
        let mut connection = Connection::open_in_memory().unwrap();
        run(&mut connection, &MIGRATIONS[..61]).unwrap();
        let opaque = [0_u8, 0xff, 0x80, 0x00];
        connection
        ...
        let error = run(&mut connection, &MIGRATIONS[..60]).unwrap_err();
        assert!(matches!(
            error,
