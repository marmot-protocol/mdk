//! Migration 0065: durable ordered account-runtime visibility outbox.
...
mod tests {
    ...
    #[test]
    fn migration_adds_an_empty_ordered_visibility_journal_after_schema_64() {
    ...
    #[test]
    fn schema_64_runner_refuses_a_visibility_journal_database_without_mutating_rows() {
        let mut connection = Connection::open_in_memory().unwrap();
        run(&mut connection, MIGRATIONS).unwrap();
        ...
        let error = run(&mut connection, &MIGRATIONS[..64]).unwrap_err();
        assert!(matches!(
            error,
