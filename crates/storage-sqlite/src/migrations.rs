...
#[path = "migrations/0063_key_package_lifecycle_privacy_journal.rs"]
mod migration_0063_key_package_lifecycle_privacy_journal;
#[path = "migrations/0064_epoch_backfill_intent_journal.rs"]
mod migration_0064_epoch_backfill_intent_journal;
#[path = "migrations/0065_account_visibility_journal.rs"]
mod migration_0065_account_visibility_journal;
#[cfg(test)]
...
const MIGRATIONS: &[Migration] = &[
    ...
    Migration {
        version: 63,
        name: "0063_key_package_lifecycle_privacy_journal",
        apply: migration_0063_key_package_lifecycle_privacy_journal::apply,
    },
    Migration {
        version: 64,
        name: "0064_epoch_backfill_intent_journal",
        apply: migration_0064_epoch_backfill_intent_journal::apply,
    },
    Migration {
        version: 65,
        name: "0065_account_visibility_journal",
        apply: migration_0065_account_visibility_journal::apply,
    },
];
...
mod tests {
    ...
    fn key_package_privacy_journal_migration_refuses_schema_51_writer() {
        ...
        run(&mut connection, &MIGRATIONS[..63]).unwrap();
        ...
        let error = run(&mut connection, &MIGRATIONS[..62]).unwrap_err();
        assert!(matches!(
            error,
    ...
    fn process_kill_mid_key_package_privacy_migration_retries_fail_closed() {
        ...
        let connection = keyed_connection(&database);
        assert_eq!(applied_name(&connection, 63).unwrap(), None);
        ...
        assert_eq!(
            applied_name(&connection, 63).unwrap().as_deref(),
            Some("0063_key_package_lifecycle_privacy_journal")
        );
        let migration_count: i64 = connection
            .query_row(
                "SELECT count(*) FROM cgka_schema_migrations WHERE version = 63",
                [],
                |row| row.get(0),
            )
            .unwrap();
