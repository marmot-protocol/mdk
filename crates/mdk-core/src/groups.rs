    ...
    Storage: MdkStorageProvider,
{
    ...
    /// Converges a member's local wire-format policy to
    /// `MIXED_CIPHERTEXT_WIRE_FORMAT_POLICY`, MDK's current default.
    ///
    /// Called from commit-emitting operations as a piggy-backed hygiene step:
    /// every rotation brings a member's local state forward so that a future
    /// `GroupContextExtensions` upgrade adding `SelfRemove` to the group's
    /// `RequiredCapabilities` finds members already able to accept SelfRemove
    /// `PublicMessage` proposals. No-op when the policy is already converged,
    /// so it's cheap to call unconditionally.
    ///
    /// Failures to persist the updated configuration are logged at `warn` and
    /// swallowed. The in-memory config is still correct for the current
    /// session; the next rotation will retry. The caller's primary operation
    /// is not affected.
    ///
    /// # Note on full-config rebuild
    ///
    /// `MlsGroup::set_configuration` replaces the entire `MlsGroupJoinConfig`.
    /// Ideally we would preserve every non-wire-format field from the group's
    /// stored config, but openmls 0.8.1 only exposes getters for three of the
    /// six fields (`wire_format_policy`, `padding_size`,
    /// `sender_ratchet_configuration`). Falling back to builder defaults for
    /// the hidden fields would regress known MDK settings
    /// (`max_past_epochs`, `use_ratchet_tree_extension`). MDK's existing
    /// invariant — every site that writes an `MlsGroupJoinConfig`
    /// (`create_group`, `join_group_from_welcome`, `try_self_remove`) rebuilds
    /// it from `self.config` — keeps the stored config in sync with the
    /// current MDK instance, so rebuilding here matches expectations for
    /// the other fields.
...
mod tests {
    ...
    /// Convergence: `self_update` normalises a stale PURE_CIPHERTEXT local
    /// policy to `MIXED_CIPHERTEXT_WIRE_FORMAT_POLICY`. The PURE precondition
    /// simulates persisted state from pre-MIXED-welcomes MDK builds — today's
    /// welcome-processing creates MIXED, but local storage from older
    /// installations can still carry PURE. Bringing members forward on
    /// rotation reduces the wire-format reconciliation burden at a future
    /// `GroupContextExtensions`-upgrade event.
    #[test]
    fn test_self_update_converges_wire_format_to_mixed_ciphertext() {
        ...
        // Force the creator's local policy to PURE_CIPHERTEXT, simulating
        // persisted state from an older MDK build that defaulted to PURE.
    ...
    mod padding_tests {
        ...
        fn test_receiver_rejects_kind_445_with_non_zero_padding() {
            ...
            fn craft_event<S: MdkStorageProvider>(
                ...
                // a real trailing region. `next_power_of_two` is the identity
                // function on a value that already IS a power of two, so when
                // the serialized payload happens to land exactly on a bucket
                // boundary the resize would be a no-op and the tail mutation
                // would clobber real MLS frame bytes (yielding a deserialize
                // failure instead of the padding-check failure under test).
                // Bump the target one bucket up in that case to guarantee at
                // least one freshly-added zero byte to mutate.
                let mut target = serialized.len().next_power_of_two().max(512);
                if target == serialized.len() {
                    target = target.checked_mul(2).expect("bucket overflow");
                }
        ...
        /// malicious admin authoring a Welcome must not be able to embed
        /// arbitrary data in the padding region as a covert channel to the
        /// new joiner. Mirrors the kind:445 enforcement in
        /// `process_mls_message` and closes the asymmetry flagged in the
        /// PR #308 review.
        ///
        /// Same side-by-side strategy as the kind:445 negative test: vary
        /// only the trailing tail byte (0 vs 0xFF) and assert divergent
        /// outcomes — success vs `InvalidWelcomeMessage`.
        #[test]
        fn test_receiver_rejects_welcome_with_non_zero_padding() {
            use nostr::EventBuilder;

            use crate::Error;

            fn craft_tampered_rumor(
                ...
                let mut decoded = BASE64
                    ...
                    .expect("welcome content is base64");
                assert!(decoded.len() >= WELCOME_PADDING_FLOOR);
                // The sender's padder may produce a buffer whose length is
                // exactly the serialized welcome's length (when that length
                // already equals a power-of-two bucket boundary), leaving no
                // trailing padding region. Append a known-zero byte first so
                // the subsequent tail mutation is guaranteed to land on a
                // padding byte rather than an MLS frame byte.
                decoded.push(0);
                *decoded.last_mut().expect("decoded buffer non-empty") = tail_byte;
                let re_encoded = BASE64.encode(&decoded);

                let mut tags: Vec<nostr::Tag> = rumor.tags.iter().cloned().collect();
                // EventBuilder appends its own tags; rebuilding from scratch
                // with the original tags preserves the relays/e/encoding tags
                // the receiver validates before reaching parse_serialized_welcome.
                let mut builder = EventBuilder::new(rumor.kind, re_encoded);
                builder = builder.tags(tags.drain(..));
                builder.build(rumor.pubkey)
            }

            fn fresh_group_with_welcome() -> (
                MDK<MdkMemoryStorage>,
                MDK<MdkMemoryStorage>,
                nostr::UnsignedEvent,
            ) {
                let admin_keys = Keys::generate();
                ...
                let admin_mdk = create_test_mdk();
                let joiner_mdk = create_test_mdk();
                let joiner_kp = create_key_package_event(&joiner_mdk, &joiner_keys);
                let create_result = admin_mdk
                    .create_group(
                        &admin_keys.public_key(),
                        vec![joiner_kp],
                        create_nostr_group_config_data(vec![admin_keys.public_key()]),
                    )
                    .expect("admin creates group");
                let rumor = create_result.welcome_rumors[0].clone();
                (admin_mdk, joiner_mdk, rumor)
            }

            // Zero-tail control: the same construction with the last byte
            // explicitly set to zero must succeed.
            let (_admin_ok, joiner_ok, rumor_ok) = fresh_group_with_welcome();
            let zero_rumor = craft_tampered_rumor(&rumor_ok, 0);
            joiner_ok
                .process_welcome(&nostr::EventId::all_zeros(), &zero_rumor)
                .expect("zero-tail control must process");

            // Non-zero tail: must be rejected as InvalidWelcomeMessage.
            let (_admin_bad, joiner_bad, rumor_bad) = fresh_group_with_welcome();
            let bad_rumor = craft_tampered_rumor(&rumor_bad, 0xFF);
            let err = joiner_bad
