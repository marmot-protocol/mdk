...
pub(crate) fn decrypt_with_exporter_secret(
    ...
    // Decrypt and authenticate (no AAD per MIP-03); any tampering or wrong key will error here
    let message_bytes = cipher.decrypt(nonce, ciphertext).map_err(|_| {
...
mod tests {
    ...
    /// Test that ChaCha20-Poly1305 encrypt/decrypt round-trips correctly (no AAD per MIP-03).
    #[test]
    fn test_chacha20poly1305_roundtrip() {
        ...
        // Manually encrypt using the same scheme as build_message_event (no AAD)
        ...
        let decrypted =
            decrypt_with_exporter_secret(&secret, &encrypted_content).expect("Should decrypt");

        assert_eq!(decrypted, plaintext);
    }

    /// Test that decryption fails with a tampered ciphertext.
    #[test]
    fn test_chacha20poly1305_tampered_ciphertext_fails() {
        ...
        let result = decrypt_with_exporter_secret(&secret, &encrypted_content);
    ...
    fn test_decrypt_rejects_invalid_base64() {
        ...
        let result = decrypt_with_exporter_secret(&secret, "!!!not-base64!!!");
    ...
    fn test_decrypt_rejects_short_nonce() {
        ...
        let result = decrypt_with_exporter_secret(&secret, &too_short);
        assert!(result.is_err());
        assert!(
            result
                .unwrap_err()
