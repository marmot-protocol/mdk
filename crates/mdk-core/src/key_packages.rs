...
mod tests {
    ...
    fn make_relay_list_event(relays: &[&str]) -> nostr::Event {
    ...
    fn test_handle_relay_list_creates_key_package_when_none_exists() {
        let mdk = create_test_mdk();
        let keys = Keys::generate();
        let event = make_relay_list_event(&["wss://relay.example.com"]);
    ...
    fn test_handle_relay_list_uses_relays_from_event() {
        ...
        let event = make_relay_list_event(&[relay_url]);
    ...
    /// When the kind 10051 event has no relay tags, a key package is still created.
    #[test]
    fn test_handle_relay_list_with_no_relays() {
        ...
        assert!(
            result.is_some(),
            "should still create a key package even with no relays"
        );
    }
}
