...
- **GroupContextExtensions upgrade path.** Three new public methods on `MDK` let client UX prompt an admin to upgrade a mixed group that has since become all-modern:
  - `group_member_capabilities(&GroupId) -> Result<Vec<MemberCapabilities>, Error>` — per-member advertised proposal types, extension types, ciphersuites, and admin flag. Any member may call it.
  ...
  - `upgrade_group_capabilities(&GroupId, proposals_to_add: &[ProposalType]) -> Result<UpdateGroupResult, Error>` — admin-only. Proposes a `GroupContextExtensions` commit adding the caller-selected proposal types to `RequiredCapabilities`. Per-proposal re-validation at send time surfaces TOCTOU via `Error::ProposalAlreadyRequired` (another admin upgraded concurrently) or `Error::ProposalNotAvailableForUpgrade { proposal, blockers }` (a new blocker appeared); the caller-bug cases surface as `Error::EmptyUpgradeSet`, `Error::ProposalNotInSupportedSet(pt)`, or `Error::NotAdmin`. All-or-nothing over the requested set — no partial commit is ever sent. Additive-only; clients drive the UX, MDK does not upgrade groups automatically. New types exported via `mdk_core::prelude`: `MemberCapabilities`, `CapabilityUpgradeStatus`, `ProposalUpgradability`, plus `ExtensionType` and `VerifiableCiphersuite` re-exports from OpenMLS.
