...
- Blocked non-admin `Add` / `Remove`-of-other / `GroupContextExtensions` proposals from riding into an admin's unrelated commit. A non-admin member could previously call `propose_add_member` against OpenMLS directly, publish the resulting `kind:445` event, and have the next admin commit sweep it from the pending-proposal store under the admin's signature, smuggling an outsider into the encrypted group with full access to the exporter secret. Fixed on both sides: `prune_unauthorized_pending_proposals` drains the pending store before `add_members` / `remove_members` / `update_group_data_extension` / `self_update` / `upgrade_group_capabilities` consume it, and `validate_committed_proposal_authorship` rejects any staged commit whose bundled proposals are not admin-authored (`Update`, `SelfRemove`, and self-targeted `Remove` are still allowed as self-scoped). New `Error::UnauthorizedProposalInCommit` variant routes through the existing `authorization_failed` failure category. Closes [marmot-protocol/marmot-security#106](https://github.com/marmot-protocol/marmot-security/issues/106). ([#317](https://github.com/marmot-protocol/mdk/pull/317))
