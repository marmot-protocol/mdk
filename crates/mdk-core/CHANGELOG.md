...
- **Security**: Fixed a replay-deduplication gap where the same encrypted MLS payload, re-wrapped in a fresh kind:445 event, would bypass the outer-event-ID dedup gate. This previously allowed an attacker without the group key to force duplicate application-message delivery and repeated commit/proposal processing for the same ciphertext. The fix records stable inner message IDs (rumor ID for application messages, content hash for commits/proposals) and performs a pre-decrypt dedup check, closing both the duplicate-delivery and decrypt-cost-amplification surfaces. Closes [marmot-protocol/marmot-security#2](https://github.com/marmot-protocol/marmot-security/issues/2). ([#316](https://github.com/marmot-protocol/mdk/pull/316))
