...
- New error variant `MissingGroupIdTag` for events missing the required group ID tag ([#116](https://github.com/marmot-protocol/mdk/pull/116))
- New error variant `InvalidGroupIdFormat` for malformed group ID values ([#116](https://github.com/marmot-protocol/mdk/pull/116))
...
- Fixed removed member commit processing to handle eviction gracefully. When a member is removed from a group and processes their removal commit, the group state is now set to `Inactive` instead of failing with a `UseAfterEviction` error. ([#137](https://github.com/marmot-protocol/mdk/pull/137))
