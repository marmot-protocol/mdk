...
- **Security (Audit Issue AE)**: Added comprehensive Nostr-based validations when processing messages per MIP-03 requirements. The `validate_event_and_extract_group_id` function now checks timestamp bounds (rejects events >5 minutes in future or >7 days old, configurable via `MdkConfig`), and enforces exactly one `h` tag requirement with proper format validation. Signature verification is handled by nostr-sdk's relay pool. This prevents misrouting messages via manipulated tags and degrading availability through abnormal timestamps. ([#128](https://github.com/marmot-protocol/mdk/pull/128))
