...
- `create_message` now auto-applies a NIP-40 `expiration` tag to the outer kind:445 wrapper event when the group has `disappearing_message_secs` set. The expiration timestamp is `rumor.created_at + duration_secs`. Callers no longer need to manually compute and pass the expiration tag for disappearing-message groups. Any caller-supplied expiration tag is replaced by the auto-computed one. ([#253](https://github.com/marmot-protocol/mdk/pull/253))
