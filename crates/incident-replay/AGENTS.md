...
- **Module:** `src/accept.rs`
  - **Role:** `accept` (fork) dispatches on the commit kind. A **group-data** fork
    ...
    matches — winner survival among them, since `ClientsConverged` pins the
    winner's group name, so the ordering search and the acceptance gate are one
    comparison and the persisted vector carries the winner check into CI. A
