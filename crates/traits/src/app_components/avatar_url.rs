...
/// Contact-policy check for fetching or rendering a group avatar URL.
///
/// This is independent from [`validate_and_normalize_group_avatar_url`]: a URL
/// may be valid group state while still being unsafe for this client to contact.
pub fn reject_unsafe_group_avatar_contact_url(raw: &str) -> Result<(), String> {
    let url = Url::parse(raw).map_err(|e| format!("group avatar URL is invalid: {e}"))?;
