...
/// Upper bound used to reject oversize relay vectors before allocating or
/// iterating entry bodies. Each entry is at most a 2-byte QUIC length prefix
/// plus [`NOSTR_RELAY_URL_MAX_LEN`] content bytes.
...
impl NostrRoutingV1 {
    pub fn new(nostr_group_id: [u8; 32], mut relays: Vec<String>) -> Result<Self, String> {
        ...
        relays.dedup();
        // Dedup can shrink a previously over-limit unique-prefix list, so
        // re-check the canonical count after sort/dedup.
        validate_nostr_relay_count(relays.len())?;
        Ok(Self {
            nostr_group_id,
            relays,
