...
use marmot_markdown::parse;

const UPDATE_ENV: &str = "MARMOT_MD_UPDATE_GOLDEN";
...
fn assert_golden(name: &str) {
    ...
    let expected = fs::read_to_string(&json_path).unwrap_or_else(|e| {
        panic!(
            "read {}: {e}\nhint: run `{UPDATE_ENV}=1 cargo test -p marmot-markdown --test golden` to create the fixture",
            json_path.display()
        )
    });
    ...
    panic!(
        "golden mismatch for `{name}`\n  fixture: {}\n  first diff at line {line}:\n    expected: {e_line}\n    actual:   {a_line}\n  hint: run `{UPDATE_ENV}=1 cargo test -p marmot-markdown --test golden` to update\n",
        json_path.display(),
    );
}
