...
`MemoryStorage` implements the full `cgka_traits::StorageProvider` aggregate for
tests, simulator runs, and ephemeral scenarios. It uses `Arc<RwLock<...>>` so
clones share one backend.
...
- Group delete must cascade messages, queued outbound intents, welcomes,
  capabilities, policies, snapshots, and group-scoped OpenMLS state.
